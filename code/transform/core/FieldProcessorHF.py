...
class FieldProcessorHF:
    ...
    def __init__(self, path_to_config_data: str = "./../Config_Data"):
        self.M4ML_schema = pd.read_csv(
            path_to_config_data + "/M4ML_schema.tsv", sep="\t"
        )
    ...
    #                 data="Not extracted", extraction_method="None", confidence=1.0
    #             )
    #         ]
    ...
    #         processed_value = self.build_HF_link(
    #             info_HF, tail_info="/blob/main/README.md"
    #         )
    ...
    def process_property_(
        ...
        special_processors = {
            "fair4ml:hasCO2eEmissions": lambda: [
                self.add_default_extraction_info("Not extracted", "None", 1.0)
            ],
        ...
        if property_name in direct_question_mappings:
            return self.find_value_in_HF(
                info_HF, direct_question_mappings[property_name]
            )
