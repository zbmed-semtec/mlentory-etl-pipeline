#!/usr/bin/env python3
"""
Native Elasticsearch Multi-Vector N-gram Enhanced Search CLI

This CLI uses Elasticsearch's native vector similarity search capabilities with n-gram enhanced vectors:
- name_vector: N-gram enhanced vector for the model name
- task_vector: N-gram enhanced vector for mlTask + keywords combined  
- creator_vector: N-gram enhanced vector for sharedBy field
- Uses the vector_multi_ngram_hf_models index for n-gram enhanced multi-vector search

Usage:
    python3 search_multi_ngram_cli_native.py "your search query"
    python3 search_multi_ngram_cli_native.py --interactive
    python3 search_multi_ngram_cli_native.py "query" --search-type hybrid --top-k 10

Example:
    Query: "BERT-based language models for biomedical text"
    N-gram enhanced multi-vector search uses enhanced name, task, and creator vectors for better partial matching
"""
...
class MultiVectorNgramSearchHandler:
    """
    Handler for native Elasticsearch multi-vector n-gram enhanced similarity search.
    
    This class uses Elasticsearch's native dense_vector fields and KNN search
    with n-gram enhanced multiple vector fields for better partial matching.
    """
    
    def __init__(self):
        """Initialize the multi-vector n-gram search handler."""
        self.es = None
    ...
    def find_multi_vector_ngram_index(self, source_index: str) -> Optional[str]:
        """
        Find the multi-vector n-gram index for a source index.
        
        Args:
            source_index: The source index name (e.g., "hf_models")
            
        Returns:
            str: The multi-vector n-gram index name if found, None otherwise
        """
        try:
            # Get all indices
            all_indices = self.es.cat.indices(format='json')
            
            # Look for multi-vector n-gram indices
            vector_index_patterns = [
                f"vector_multi_ngram_{source_index}",
            ]
            ...
            print(f"⚠️  No multi-vector n-gram index found for {source_index}")
            ...
            print(f"❌ Error finding multi-vector n-gram index: {e}")
            return None
    ...
    def multi_vector_ngram_search(self, query: str, source_index: str, top_k: int = 5) -> List[Dict[str, Any]]:
        """
        Perform native multi-vector n-gram enhanced search using Elasticsearch's KNN with multiple vector fields.
        ...
        try:
            # Find the multi-vector n-gram index
            vector_index = self.find_multi_vector_ngram_index(source_index)
            if not vector_index:
                print(f"❌ No multi-vector n-gram index found for {source_index}")
                return []
            
            # Generate query vector
            query_vector = self.embedding_service.encode_text(query)
            
            # Build native KNN search query with multiple n-gram enhanced vector fields
            search_body = {
                "size": top_k,
                "query": {
                    "bool": {
                        "should": [
                            # Name vector search (n-gram enhanced)
                            {
                                "knn": {
                                    "field": "name_vector",
                            ...
                            # Task vector search (n-gram enhanced)
                            {
                                "knn": {
                                    "field": "task_vector",
                                    "query_vector": query_vector,
                            ...
                            # Creator vector search (n-gram enhanced)
                            {
                                "knn": {
            ...
            for hit in response["hits"]["hits"]:
                result = {
                    ...
                    "model_data": hit["_source"],
                    "db_identifier": hit["_source"].get("db_identifier", ""),
                    "search_type": "multi_vector_ngram"
                }
                results.append(result)
            ...
            print(f"❌ Native multi-vector n-gram search failed: {e}")
            return []
    ...
    def hybrid_search(self, query: str, source_index: str, top_k: int = 5, 
        ...
        """
        Perform native hybrid search combining text and multi-vector n-gram enhanced search.
        ...
        try:
            ...
            vector_index = self.find_multi_vector_ngram_index(source_index)
            if not vector_index:
                print(f"❌ No multi-vector n-gram index found for {source_index}, falling back to text search")
                return self.text_search(query, source_index, top_k)
                ...
                "query": {
                    "bool": {
                        "should": [
                            ...
                            # Multi-vector n-gram enhanced search with weight
                            {
                                "bool": {
                                    "should": [
                                        # Name vector search (n-gram enhanced)
    ...
    def search_all_methods(self, query: str, index_name: str, top_k: int = 5, 
        ...
        # Multi-Vector N-gram Enhanced Search
        try:
            start_time = time.time()
            vector_results = self.multi_vector_ngram_search(query, index_name, top_k)
            ...
            results["multi_vector_ngram_search"] = {
            ...
            if not vector_results:
                print(f"⚠️  Multi-vector n-gram search returned no results. Index may not exist.")
                print(f"💡 Make sure the multi-vector n-gram index exists with name_vector, task_vector, creator_vector fields.")
                
        except Exception as e:
            print(f"❌ Multi-vector n-gram search failed: {e}")
            print(f"💡 Make sure the multi-vector n-gram index exists with name_vector, task_vector, creator_vector fields.")
            results["multi_vector_ngram_search"] = {"results": [], "time": 0, "count": 0, "ground_truth": {}}
...
def display_results_multi_ngram_native(results: Dict[str, Any], query: str):
    """
    Display search results in a formatted way for multi-vector n-gram native search with ground truth comparison.
    
    Args:
        results: Results dictionary from search_all_methods
        query: Original search query
    """
    print(f"\n🔍 Multi-Vector N-gram Enhanced Native Elasticsearch Search Results for: '{query}'")
    print("=" * 80)
    ...
    # Multi-Vector N-gram Enhanced Search Results
    if "multi_vector_ngram_search" in results and results["multi_vector_ngram_search"]["results"]:
        print(f"\n📊 NATIVE MULTI-VECTOR N-GRAM ENHANCED SEARCH:")
        print("-" * 30)
        
        # Display ground truth metrics
        gt_metrics = results["multi_vector_ngram_search"].get("ground_truth", {})
        if gt_metrics.get("has_ground_truth", False):
            print(f"🎯 Ground Truth Match: {gt_metrics['found_count']} of {len(results['multi_vector_ngram_search']['results'])} returned models match ground truth")
        else:
            print("🎯 No ground truth available for this query")
        
        print()
        for i, result in enumerate(results["multi_vector_ngram_search"]["results"], 1):
            model_data = result["model_data"]
            score = result["score"]
            model_name = model_data.get('name', 'Unknown')
            ...
            print(f"   {i}. {gt_marker} {model_name} ({model_data.get('platform', 'Unknown')})")
            print(f"      N-gram Enhanced Multi-Vector Score: {score:.4f}")
            print(f"      ML Tasks: {model_data.get('mlTask', 'N/A')}")
            # Construct HuggingFace URL for hf_models
            if model_name and "/" in model_name:  # HuggingFace models have format "org/model-name"
                source_url = f"https://huggingface.co/{model_name}"
    ...
    if results:
        ...
        if "multi_vector_ngram_search" in results:
            print(f"   Native Multi-Vector N-gram Search: {results['multi_vector_ngram_search']['time']:.3f}s")
        ...
        for search_type in ["text_search", "multi_vector_ngram_search", "hybrid_search"]:
...
def interactive_mode():
    """
    Run the CLI in interactive mode with native multi-vector n-gram Elasticsearch search.
    """
    # Start logging for interactive mode
    start_console_logging("logs", "search_multi_ngram_cli_native_interactive")
    
    print("🚀 Native Multi-Vector N-gram Enhanced Elasticsearch Search CLI - Interactive Mode")
    print("=" * 80)
    ...
    # Initialize the multi-vector n-gram search handler and ground truth parser
    try:
        handler = MultiVectorNgramSearchHandler()
        ground_truth_parser = GroundTruthParser()
    except Exception as e:
        print(f"❌ Failed to initialize multi-vector n-gram search handler: {e}")
        return
    
    while True:
        try:
            user_input = input("🔍 Multi-Vector N-gram Search> ").strip()
            ...
            if search_type == "all":
                ...
                display_results_multi_ngram_native(results, query)
                ...
                display_results_multi_ngram_native({"text_search": {"results": results, "ground_truth": gt_metrics}}, query)
            elif search_type == "vector":
                results = handler.multi_vector_ngram_search(query, index_name, top_k=5)
                # Calculate ground truth metrics for single search type
                gt_metrics = ground_truth_parser.calculate_recall(query, results)
                display_results_multi_ngram_native({"multi_vector_ngram_search": {"results": results, "ground_truth": gt_metrics}}, query)
            elif search_type == "hybrid":
                results = handler.hybrid_search(query, index_name, top_k=5)
                # Calculate ground truth metrics for single search type
                gt_metrics = ground_truth_parser.calculate_recall(query, results)
                display_results_multi_ngram_native({"hybrid_search": {"results": results, "ground_truth": gt_metrics}}, query)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Native Multi-Vector N-gram Enhanced Elasticsearch Search CLI",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 search_multi_ngram_cli_native.py "BERT model"
  python3 search_multi_ngram_cli_native.py "image classification" --search-type hybrid
  python3 search_multi_ngram_cli_native.py "huggingface" --search-type vector --top-k 5
  python3 search_multi_ngram_cli_native.py --interactive
        """
    )
    ...
    start_console_logging("logs", "search_multi_ngram_cli_native_single")
    ...
    try:
        ...
        if args.search_type == "all":
            ...
            display_results_multi_ngram_native(results, args.query)
            ...
            display_results_multi_ngram_native({"text_search": {"results": results, "ground_truth": gt_metrics}}, args.query)
        elif args.search_type == "vector":
            results = handler.multi_vector_ngram_search(args.query, args.index, args.top_k)
            # Calculate ground truth metrics for single search type
            gt_metrics = ground_truth_parser.calculate_recall(args.query, results)
            display_results_multi_ngram_native({"multi_vector_ngram_search": {"results": results, "ground_truth": gt_metrics}}, args.query)
        elif args.search_type == "hybrid":
            results = handler.hybrid_search(args.query, args.index, args.top_k)
            # Calculate ground truth metrics for single search type
            gt_metrics = ground_truth_parser.calculate_recall(args.query, results)
            display_results_multi_ngram_native({"hybrid_search": {"results": results, "ground_truth": gt_metrics}}, args.query)
