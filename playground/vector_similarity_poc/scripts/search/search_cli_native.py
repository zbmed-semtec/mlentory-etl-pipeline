#!/usr/bin/env python3
"""
Native Elasticsearch Vector Similarity Search CLI

This CLI uses Elasticsearch's native vector similarity search capabilities:
- Uses dense_vector field types for optimal performance
- Leverages Elasticsearch's built-in KNN search
- Provides native hybrid search combining text and vector search
- Uses the same indices as the original search_cli.py

Usage:
    python3 search_cli_native.py "your search query"
    python3 search_cli_native.py --interactive
    python3 search_cli_native.py "query" --search-type hybrid --top-k 10

Example:
    Query: "BERT-based language models for biomedical text"
    Native hybrid search combines text matching with semantic similarity
"""
...
class NativeVectorSearchHandler:
    """
    Handler for native Elasticsearch vector similarity search.
    
    This class uses Elasticsearch's native dense_vector fields and KNN search
    for optimal performance and built-in hybrid search capabilities.
    """
    
    def __init__(self):
        """Initialize the native vector search handler."""
        self.es = None
    ...
    def find_native_vector_index(self, source_index: str) -> Optional[str]:
        """
        Find the native vector index for a source index.
        
        Args:
            source_index: The source index name (e.g., "hf_models")
            
        Returns:
            str: The native vector index name if found, None otherwise
        """
        try:
            # Get all indices
            all_indices = self.es.cat.indices(format='json')
            
            # Look for native vector indices (dense_vector fields)
            # These would typically be named like: "vector_native_hf_models" or similar
            vector_index_patterns = [
                f"vector_{source_index}",
            ]
            ...
            # If no native vector index found, check if the original index has dense_vector fields
            if self._check_for_dense_vector_fields(source_index):
                return source_index
            
            print(f"⚠️  No native vector index found for {source_index}")
            ...
            print(f"❌ Error finding native vector index: {e}")
            return None
    ...
    def _check_for_dense_vector_fields(self, index_name: str) -> bool:
        ...
        Returns:
            bool: True if the index has dense_vector fields
        """
        try:
            mapping = self.es.indices.get_mapping(index=index_name)
            properties = mapping[index_name]['mappings']['properties']
            
            # Check for dense_vector fields
            ...
            print(f"⚠️  Could not check mapping for {index_name}: {e}")
            return False
    ...
    def vector_search(self, query: str, source_index: str, top_k: int = 5) -> List[Dict[str, Any]]:
        """
        Perform native vector search using Elasticsearch's KNN.
        ...
        try:
            ...
            vector_index = self.find_native_vector_index(source_index)
            if not vector_index:
                print(f"❌ No vector index found for {source_index}")
                return []
            
            # Generate query vector
            query_vector = self.embedding_service.encode_text(query)
            
            # Build native KNN search query
            search_body = {
                "size": top_k,
                "knn": {
                    "field": "model_vector",
            ...
            print(f"❌ Native vector search failed: {e}")
            return []
    ...
    def hybrid_search(self, query: str, source_index: str, top_k: int = 5, 
        ...
        """
        Perform native hybrid search using Elasticsearch's bool query with KNN.
        
        This uses Elasticsearch's native capability to combine text and vector search
        in a single query, with automatic score combination.
        ...
        try:
            ...
            vector_index = self.find_native_vector_index(source_index)
            if not vector_index:
                print(f"❌ No vector index found for {source_index}, falling back to text search")
                return self.text_search(query, source_index, top_k)
    ...
    def search_all_methods(self, query: str, index_name: str, top_k: int = 5, 
        ...
        try:
            start_time = time.time()
            vector_results = self.vector_search(query, index_name, top_k)
            ...
            if not vector_results:
                print(f"⚠️  Vector search returned no results. Index may not exist.")
                print(f"💡 Make sure the native vector index exists with dense_vector fields.")
                
        except Exception as e:
            print(f"❌ Vector search failed: {e}")
            print(f"💡 Make sure the native vector index exists with dense_vector fields.")
            results["vector_search"] = {"results": [], "time": 0, "count": 0, "ground_truth": {}}
...
def display_results_native(results: Dict[str, Any], query: str):
    """
    Display search results in a formatted way for native search with ground truth comparison.
    
    Args:
        results: Results dictionary from search_all_methods
        query: Original search query
    """
    print(f"\n🔍 Native Elasticsearch Search Results for: '{query}'")
    print("=" * 60)
    ...
    if "vector_search" in results and results["vector_search"]["results"]:
        print(f"\n📊 NATIVE VECTOR SEARCH:")
        print("-" * 30)
        
        # Display ground truth metrics
        gt_metrics = results["vector_search"].get("ground_truth", {})
        if gt_metrics.get("has_ground_truth", False):
            print(f"🎯 Ground Truth Match: {gt_metrics['found_count']} of {len(results['vector_search']['results'])} returned models match ground truth")
        else:
            print("🎯 No ground truth available for this query")
        
        print()
        for i, result in enumerate(results["vector_search"]["results"], 1):
            model_data = result["model_data"]
            score = result["score"]
            model_name = model_data.get('name', 'Unknown')
    ...
    if results:
        ...
        if "vector_search" in results:
            print(f"   Native Vector Search: {results['vector_search']['time']:.3f}s")
        ...
        for search_type in ["text_search", "vector_search", "hybrid_search"]:
...
def interactive_mode():
    """
    Run the CLI in interactive mode with native Elasticsearch search.
    """
    # Start logging for interactive mode
    start_console_logging("logs", "search_cli_native_interactive")
    
    print("🚀 Native Elasticsearch Vector Search CLI - Interactive Mode")
    print("=" * 60)
    ...
    # Initialize the native search handler and ground truth parser
    try:
        handler = NativeVectorSearchHandler()
        ground_truth_parser = GroundTruthParser()
    except Exception as e:
        print(f"❌ Failed to initialize native search handler: {e}")
        return
    
    while True:
        try:
            user_input = input("🔍 Native Search> ").strip()
            ...
            if search_type == "all":
                ...
                display_results_native(results, query)
                ...
                display_results_native({"text_search": {"results": results, "ground_truth": gt_metrics}}, query)
            elif search_type == "vector":
                results = handler.vector_search(query, index_name, top_k=5)
                # Calculate ground truth metrics for single search type
                gt_metrics = ground_truth_parser.calculate_recall(query, results)
                display_results_native({"vector_search": {"results": results, "ground_truth": gt_metrics}}, query)
            elif search_type == "hybrid":
                results = handler.hybrid_search(query, index_name, top_k=5)
                # Calculate ground truth metrics for single search type
                gt_metrics = ground_truth_parser.calculate_recall(query, results)
                display_results_native({"hybrid_search": {"results": results, "ground_truth": gt_metrics}}, query)
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Native Elasticsearch Vector Similarity Search CLI",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  python3 search_cli_native.py "BERT model"
  python3 search_cli_native.py "image classification" --search-type hybrid
  python3 search_cli_native.py "huggingface" --search-type vector --top-k 5
  python3 search_cli_native.py --interactive
        """
    )
    ...
    start_console_logging("logs", "search_cli_native_single")
    ...
    try:
        ...
        if args.search_type == "all":
            ...
            display_results_native(results, args.query)
            ...
            display_results_native({"text_search": {"results": results, "ground_truth": gt_metrics}}, args.query)
        elif args.search_type == "vector":
            results = handler.vector_search(args.query, args.index, args.top_k)
            # Calculate ground truth metrics for single search type
            gt_metrics = ground_truth_parser.calculate_recall(args.query, results)
            display_results_native({"vector_search": {"results": results, "ground_truth": gt_metrics}}, args.query)
        elif args.search_type == "hybrid":
            results = handler.hybrid_search(args.query, args.index, args.top_k)
            # Calculate ground truth metrics for single search type
            gt_metrics = ground_truth_parser.calculate_recall(args.query, results)
            display_results_native({"hybrid_search": {"results": results, "ground_truth": gt_metrics}}, args.query)
