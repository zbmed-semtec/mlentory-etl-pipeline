...
class TestHFDatasetManager:
    ...
    def test_get_model_metadata_dataset_without_update(
        ...
        assert list(result.columns) == [
            "modelId", "author", "last_modified", "downloads", "likes",
            "pipeline_tag", "tags", "task_categories", "createdAt", "card"
        ]
        mock_load_dataset.assert_called_once_with("librarian-bots/model_cards_with_metadata")
    ...
    def test_get_recent_models_metadata(self, mock_model_card):
        ...
        # Execute
        result = manager.get_recent_models_metadata(limit=3, latest_modification=latest_modification)
        # Assert
        assert isinstance(result, pd.DataFrame)
        assert len(result) == 3
