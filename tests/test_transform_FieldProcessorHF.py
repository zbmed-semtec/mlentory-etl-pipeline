...
class TestFieldProcessorHF:
    ...
    def test_process_row(self,setup_field_processor:FieldProcessorHF):
        ...
        assert processed_row['schema.org:distribution'] == [{'data': 'https://huggingface.co/model_name', 'extraction_method': 'Built in transform stage', 'confidence': 1.0}]
        assert processed_row['schema.org:memoryRequirements'] == [{'data': '4GB', 'extraction_method': 'parsed', 'confidence': 1.0}]
        assert processed_row['schema.org:processorRequirements'] == [{'confidence': 1.0, 'data': 'Ryzen5', 'extraction_method': 'parsed'}]
        assert processed_row['schema.org:releaseNotes'] == [{'data': 'release notes', 'extraction_method': 'parsed', 'confidence': 1.0}]
        assert processed_row['schema.org:softwareRequirements'] == [{'data': ['pytorch'], 'extraction_method': 'parsed', 'confidence': 1.0}, {'data': 'Python', 'extraction_method': 'Added in transform stage', 'confidence': 1.0}]
        assert processed_row['schema.org:storageRequirements'] == [{'data': '4GB', 'extraction_method': 'parsed', 'confidence': 1.0}]
        ...
        assert processed_row['schema.org:author'] == [{'data': ['author1'], 'extraction_method': 'parsed', 'confidence': 1.0}]
        assert processed_row['schema.org:dateCreated'] == [{'data': '2022-01-01', 'extraction_method': 'parsed', 'confidence': 1.0}]
        assert processed_row['schema.org:dateModified'] == [{'data': '2022-02-01', 'extraction_method': 'parsed', 'confidence': 1.0}]
        assert processed_row['schema.org:datePublished'] == [{'data': '2022-01-01', 'extraction_method': 'parsed', 'confidence': 1.0}]
        assert processed_row['schema.org:discussionUrl'] == [{'data': 'https://huggingface.co/model_name/discussions', 'extraction_method': 'Built in transform stage', 'confidence': 1.0}]
        assert processed_row['schema.org:funding'] == [{'data': ['funding1'], 'extraction_method': 'parsed', 'confidence': 1.0}]
        assert processed_row['schema.org:license'] == [{'data': ['license1'], 'extraction_method': 'parsed', 'confidence': 1.0}]
        assert processed_row['schema.org:maintainer'] == [{'data': 'author', 'extraction_method': 'parsed', 'confidence': 1.0}]
        assert processed_row['schema.org:version'] == [{'data': '1.0.0', 'extraction_method': 'parsed', 'confidence': 1.0}]
        assert processed_row['schema.org:identifier'] == [{'data': 'https://huggingface.co/model_name', 'extraction_method': 'Built in transform stage', 'confidence': 1.0}]
        assert processed_row['schema.org:name'] == [{'data': 'model_name', 'extraction_method': 'parsed', 'confidence': 1.0}]
        assert processed_row['schema.org:url'] == [{'data': 'https://huggingface.co/model_name', 'extraction_method': 'Built in transform stage', 'confidence': 1.0}]
